...
public interface EnrolmentRepository extends JpaRepository<Enrolment, Long> {
	...
	// All course grades for the student in the given academic week (newest enrolment first). Used to prefer non-SRW when SRW is also enrolled.
	// endWeek+1 grace period matches the login check (checkEnrolmentTime) so students can see content in their final grace week.
	@Query(value = "SELECT co.grade, en.registerDate FROM Enrolment en " +
			"JOIN Class cl ON en.clazzId = cl.id " +
			"JOIN Course co ON cl.courseId = co.id " +
			"JOIN jac.Cycle cy ON co.cycleId = cy.id " +
			"WHERE en.studentId = :studentId " +
			"AND cy.year = :year " +
			"AND :week >= en.startWeek " +
			"AND :week <= (en.endWeek + 1) " +
			"AND en.old = 0 " +
			"AND en.cancelled = 0 " +
			"ORDER BY en.registerDate DESC", nativeQuery = true)
	List<Object[]> findEnrolmentGradesForWeekByStudentId(@Param("studentId") long studentId, @Param("year") int year, @Param("week") int week);

	@Query(value = "SELECT co.grade FROM Enrolment en " +
			"JOIN Class cl ON en.clazzId = cl.id " +
			"JOIN Course co ON cl.courseId = co.id " +
			"WHERE en.studentId = :studentId " +
			"AND en.old = 0 " +
			"AND en.cancelled = 0 " +
			"ORDER BY en.registerDate DESC, en.id DESC " +
			"LIMIT 1", nativeQuery = true)
	String findLatestEnrolmentGradeByStudentId(@Param("studentId") long studentId);

	// return book ids linked to the latest 3 enrolments by student id
	// @Query(value = "SELECT DISTINCT m.bookId FROM Material m " +
	// 		"JOIN (" +
	// 		"    SELECT latestInvoice.invoiceId FROM (" +
	// 		"        SELECT en.invoiceId, MAX(en.registerDate) AS maxRegisterDate, MAX(en.id) AS maxEnrolmentId " +
	// 		"        FROM Enrolment en " +
	// 		"        WHERE en.studentId = :studentId " +
	// 		"        AND en.old = 0 " +
	// 		"        AND en.invoiceId IS NOT NULL " +
	// 		"        GROUP BY en.invoiceId" +
	// 		"    ) latestInvoice " +
	// 		"    ORDER BY latestInvoice.maxRegisterDate DESC, latestInvoice.maxEnrolmentId DESC " +
	// 		"    LIMIT 3" +
	// 		") latestEnrol ON latestEnrol.invoiceId = m.invoiceId", nativeQuery = true)
	// List<Long> findBookIdsForLatest3EnrolmentsByStudentId(@Param("studentId") long studentId);
